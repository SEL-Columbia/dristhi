...
public class EventsListener {
    ...
    @Autowired
    public EventsListener(EventsRouter eventsRouter,
    		ConfigService configService, AllEvents allEvents,ErrorTraceService errorTraceService) {
        this.configService = configService;
        this.errorTraceService = errorTraceService;
        this.eventsRouter = eventsRouter;
        this.allEvents=allEvents;
		this.configService.registerAppStateToken(AllConstants.Config.EVENTS_PARSER_LAST_PROCESSED_EVENT, 
			0, "Token to keep track of events processed for client n event parsing and schedule handling", true);
    }
        ...
        try {
            ...
            for (Event event : events) {
            	try{
	            	...
	            	configService.updateAppStateToken(AllConstants.Config.EVENTS_PARSER_LAST_PROCESSED_EVENT, event.getVersion());
            	}
            	catch(Exception e){
            		e.printStackTrace();
