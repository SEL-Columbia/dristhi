...
public class PatientService extends OpenmrsService {
					...
					if (event.getEventType().equals("Birth Registration")) {
						...
						for (Obs obs2 : obs) {
							...
							if (obs2 != null && obs2.getFieldType().equals("formsubmissionField")
								...
								ArrayList<String> locationsHierarchy = openmrsLocationService
								        .getLocationsHierarchy(new Gson().toJson(openmrsLocationService
								                .getLocationTreeOf(obs2.getValue().toString())));
								
								jao.put("address4", openmrsLocationService.getLocation(obs2.getValue().toString()).getName());
	...
	public JSONObject updatePatient(Client c, String uuid) throws JSONException {
		JSONObject p = new JSONObject();
		JSONObject personToUpdate = convertBaseEntityToOpenmrsJson(c);
		personToUpdate.remove("addresses");
		personToUpdate.remove("names");
		p.put("person", convertBaseEntityToOpenmrsJson(c).remove("addresses"));
		JSONArray ids = new JSONArray();
		if (c.getIdentifiers() != null) {
	...
	public JSONObject updatePersonName(JSONObject patientObject, Client client) throws JSONException {
		...
		return new JSONObject(HttpUtil.post(getURL() + "/" + url, "", requestBody.toString(), OPENMRS_USER, OPENMRS_PWD)
		        .body());
	...
	public JSONObject updatePersonAddressAndName(Event addressUpdateEvent) throws JSONException {
		...
		for (Obs obs2 : obs) {
			...
			if (obs2 != null && obs2.getFieldType().equals("formsubmissionField")
				...
				LocationTree locationTree = openmrsLocationService.getLocationTreeWithUpperHierachyOf(obs2.getValue()
				        .toString());
				...
				if (!addressObject.getString("address4").equals(clientAddress4)) {
					...
					requestBody.put(
					    "countyDistrict",
					    locationsHierarchyMap.containsKey(AllowedLevels.DISTRICT.toString()) ? locationsHierarchyMap
					            .get(AllowedLevels.DISTRICT.toString()) : "");
					requestBody.put(
					    "stateProvince",
					    locationsHierarchyMap.containsKey(AllowedLevels.PROVINCE.toString()) ? locationsHierarchyMap
					            .get(AllowedLevels.PROVINCE.toString()) : "");
					requestBody.put(
					    "country",
					    locationsHierarchyMap.containsKey(AllowedLevels.COUNTRY.toString()) ? locationsHierarchyMap
					            .get(AllowedLevels.COUNTRY.toString()) : "");
				}
