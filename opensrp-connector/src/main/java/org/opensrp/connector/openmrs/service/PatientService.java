...
public class PatientService extends OpenmrsService {
	...
	public JSONObject createPatientRelationShip(String personB, String personA, String relationshipType)
		...
		return new JSONObject(
				HttpUtil.post(getURL() + "/" + PATIENT_CREATE_RELATIONSHIP_URL, "", o.toString(), OPENMRS_USER, OPENMRS_PWD)
						.body());
	}
						...
						for (Obs obs2 : obs) {
							...
							if (obs2 != null && obs2.getFieldType().equals("formsubmissionField") && obs2
								...
								ArrayList<String> locationsHierarchy = openmrsLocationService.getLocationsHierarchy(
										new Gson().toJson(openmrsLocationService
		...
		for (Obs obs2 : obs) {
			...
			if (obs2 != null && obs2.getFieldType().equals("formsubmissionField") && obs2.getFormSubmissionField()
				...
				if (!addressObject.getString("address4").equals(clientAddress4)) {
					...
					requestBody.put("countyDistrict", locationsHierarchy.get(0));
					requestBody.put("stateProvince", locationsHierarchy.get(1));
					requestBody.put("country", locationsHierarchy.get(2));
				}
	...
	public void updateIdentifiers(String personUUID, Client c) throws JSONException {
		...
		for (int j = 0; j < identifiers.length(); j++) {
			...
			String identifierType = idObject.getString("display");
			String identifierUuid = idObject.getString("uuid");
			String[] fn = identifierType.split("\\=");
			identifierType = fn[0].replaceAll(" ", "");
			String identifierValue = fn[1].replaceAll(" ", "");

			if (idsMap.containsKey(identifierType)) {

				if (!identifierValue.equals(idsMap.get(identifierType))) {
