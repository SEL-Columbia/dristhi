...
DECLARE
  v_date DATE;
  anm_report_data_row report.anm_report_data%ROWTYPE;
BEGIN
  FOR anm_report_data_row IN SELECT * FROM anm_report.anm_report_data LOOP
    SELECT date_ INTO v_date FROM report.dim_date WHERE id = anm_report_data_row.date_;
...
ALTER TABLE anm_report.anm_report_data DROP CONSTRAINT FK_SP_DD_ID;
ALTER TABLE anm_report.anm_report_data DROP COLUMN date_;
