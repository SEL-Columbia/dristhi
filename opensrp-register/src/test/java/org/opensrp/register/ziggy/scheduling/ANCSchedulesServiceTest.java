...
import org.opensrp.register.ziggy.service.FastForwardScheduleTestBase;
...
public class ANCSchedulesServiceTest extends BaseUnitTest {
    ...
    public void shouldEnrollMotherIntoSchedules() {
        ...
        ancSchedulesService.enrollMother("Case X", lmp, "fs1");

        verifyNonANCScheduleEnrollments(lmp, "fs1");
        verify(scheduler).enrollIntoSchedule("Case X", SCHEDULE_ANC, "ANC 1", lmp.toString(), "fs1");
    ...
    public void shouldEnrollMotherInTT2ScheduleIfTT1IsProvided() throws Exception {
        fakeIt(parse("2012-01-01"));
        when(scheduler.getEnrollmentRecord("entity id 1", "TT 1")).thenReturn(
                new EnrollmentRecord("entity id 1", "TT 1", "TT 1", null, null, null, null, null, null, null));

        ancSchedulesService.ttVisitHasHappened("entity id 1", "ANM 1", "tt1", "2012-01-01", "fs1");

        verify(scheduler).fullfillMilestoneAndCloseAlert(eq("entity id 1"), eq("ANM 1"), eq("TT 1"), eq("TT 1"), eq(today()), eq("fs1"));
        verify(scheduler).enrollIntoSchedule("entity id 1", "TT 2", "2012-01-01", "fs1");
    }
    ...
    public void shouldFullFillTT1AndTT2IfTTBoosterIsProvided() throws Exception {
        ...
        when(scheduler.getEnrollmentRecord("entity id 1", "TT 1")).thenReturn(
                new EnrollmentRecord("entity id 1", "TT 1", "TT 1", null, null, null, null, null, null, null));
        when(scheduler.getEnrollmentRecord("entity id 1", "TT 2")).thenReturn(
                new EnrollmentRecord("entity id 1", "TT 2", "TT 2", null, null, null, null, null, null, null));

        ancSchedulesService.ttVisitHasHappened("entity id 1", "ANM 1", "ttbooster", "2012-01-01", "fs1");

//       / verify(actionService).markAlertAsInactive("ANM 1", "entity id 1", "TT 1");
        verify(scheduler).unEnrollFromSchedule("entity id 1", "ANM 1", "TT 1", "fs1");
        verifyZeroInteractions(scheduler);
    }

    @Test
    public void shouldNotEnrollMotherInTT2ScheduleIfSomeOtherThingIsProvided() throws Exception {
        ancSchedulesService.ttVisitHasHappened("entity id 1", "ANM 1", "some other", "2012-01-01", "fs1");
    ...
    public void shouldNotFulfillANCIfANCScheduleIsAlreadyOver() throws Exception {
        ...
        when(scheduler.getEnrollmentRecord("Entity X", SCHEDULE_ANC)).thenReturn(null);

        ancSchedulesService.ancVisitHasHappened("Entity X", "ANM 1", 2, "2012-01-23", "fs1");
    ...
    public void shouldUnenrollAMotherFromAllOpenSchedulesAndRaiseDeleteAllAlertActionDuringClose() {
        ...
        ancSchedulesService.unEnrollFromAllSchedules("Case X", "fs1");

        verify(scheduler).unEnrollFromAllSchedules("Case X", "fs1");
    }

    @Test
    public void shouldFulfillHbTest1WhenHbTestIsDone() {
        when(scheduler.getEnrollmentRecord("Case X", "Hb Test 1"))
                .thenReturn(new EnrollmentRecord("Case X", "Hb Test 1", "HB Test 1", null, null, null, null, null, null, null));

        ancSchedulesService.hbTestDone("Case X", "ANM 1", "2013-01-01", null, parse("2012-09-01"), "fs1");

        verify(scheduler).fullfillMilestoneAndCloseAlert(eq("Case X"), eq("ANM 1"), eq("Hb Test 1"), eq("Hb Test 1"), eq(parse("2013-01-01")), eq("fs1"));
    }
    ...
    public void shouldEnrollANCToHbFollowupTestWhenHbTestIsDoneAndSheIsAnaemic() {
        ...
        ancSchedulesService.hbTestDone("Case X", "ANM 1", "2013-01-01", "Anaemic", null, "fs1");

        verify(scheduler).enrollIntoSchedule("Case X", "Hb Followup Test", "2013-01-01", "fs1");
    }
    ...
    public void shouldEnrollANCToHbTest2WhenHbTestIsDoneAndSheIsNotAnaemic() {
        ...
        verify(scheduler).enrollIntoSchedule("Case X", "Hb Test 2", "2012-09-01", "fs1");
    }

    @Test
    public void shouldFulfillHbFollowupTestWhenHbTestIsDone() {
        when(scheduler.isNotEnrolled("Case X", "Hb Test 1")).thenReturn(true);
        when(scheduler.getEnrollmentRecord("Case X", "Hb Test 1")).thenReturn(null);
        when(scheduler.getEnrollmentRecord("Case X", "Hb Followup Test"))
                .thenReturn(new EnrollmentRecord("Case X", "Hb Followup Test", "Hb Followup Test", null, null, null, null, null, null, null));

        ancSchedulesService.hbTestDone("Case X", "ANM 1", "2013-01-01", "Anaemic", parse("2012-09-01"), "fs1");

        verify(scheduler).fullfillMilestoneAndCloseAlert(eq("Case X"), eq("ANM 1"), eq("Hb Followup Test"), eq("Hb Followup Test"), eq(parse("2013-01-01")), eq("fs1"));
    }
    ...
    public void shouldFulfillHbTest2WhenHbTestIsDoneAndFollowupIsFulfilledAndDateIs28WeeksAfterLMP() {
        ...
        when(scheduler.getEnrollmentRecord("Case X", "Hb Followup Test"))
                .thenReturn(new EnrollmentRecord("Case X", "Hb Followup Test", "Hb Followup Test", null, null, null, null, null, null, null));
        when(scheduler.getEnrollmentRecord("Case X", "Hb Test 2"))
                .thenReturn(new EnrollmentRecord("Case X", "Hb Test 2", "Hb Test 2", null, null, null, null, null, null, null));

        ancSchedulesService.hbTestDone("Case X", "ANM 1", "2013-04-16", "Anaemic", parse("2012-09-01"), "fs1");

        verify(scheduler).fullfillMilestoneAndCloseAlert(eq("Case X"), eq("ANM 1"), eq("Hb Test 2"), eq("Hb Test 2"), eq(parse("2013-04-16")), eq("fs1"));
    }

    @Test
    public void shouldFulfillScheduleWhenDeliveryHasBeenPlanned() {
        when(scheduler.getEnrollmentRecord("Case X", "Delivery Plan"))
                .thenReturn(new EnrollmentRecord("Case X", "Delivery Plan", "Delivery Plan", null, null, null, null, null, null, null));

        ancSchedulesService.deliveryHasBeenPlanned("Case X", "ANM 1", "2013-01-01", "fs1");

        verify(scheduler).fullfillMilestoneAndCloseAlert(eq("Case X"), eq("ANM 1"), eq("Delivery Plan"), eq("Delivery Plan"), eq(parse("2013-01-01")), eq("fs1"));
    }
    ...
    public void shouldEnrollToHbTest2WhenHbTestIsDoneAndFollowupIsFulfilledAndDateIsNot28WeeksAfterLMP() {
        ...
        ancSchedulesService.hbTestDone("Case X", "ANM 1", "2013-02-16", "Anaemic", parse("2012-09-01"), "fs1");

        verify(scheduler, times(0)).fullfillMilestoneAndCloseAlert(eq("Case X"), eq("ANM 1"), eq("Hb Test 2"), eq(parse("2013-04-16")), eq("fs1"));
        verify(scheduler).enrollIntoSchedule("Case X", "Hb Test 2", "2012-09-01", "fs1");
    }
    ...
    public void shouldFulfillHbTest2WhenHbTestIsDoneAndItsTheActiveMilestone() {
        ...
        when(scheduler.getEnrollmentRecord("Case X", "Hb Test 1")).thenReturn(null);
        when(scheduler.isNotEnrolled("Case X", "Hb Followup Test")).thenReturn(true);
        when(scheduler.getEnrollmentRecord("Case X", "Hb Followup Test")).thenReturn(null);
        when(scheduler.getEnrollmentRecord("Case X", "Hb Test 2"))
                .thenReturn(new EnrollmentRecord("Case X", "Hb Test 2", "Hb Test 2", null, null, null, null, null, null, null));
    ...
    private void assertEnrollmentIntoMilestoneBasedOnDate(LocalDate enrollmentDate, String expectedMilestone, int wantedNumberOfInvocations) throws Exception {
        ...
        verifyNonANCScheduleEnrollments(lmp, "fs1");
        verify(scheduler, times(wantedNumberOfInvocations)).enrollIntoSchedule("Case X", SCHEDULE_ANC, expectedMilestone, lmp.toString(), "fs1");
    }
   ...
   /* private EnrollmentsQuery queryFor(final String externalId) {
    ...
    private void verifyNonANCScheduleEnrollments(LocalDate lmp, String formSubmissionId) {
        verify(scheduler).enrollIntoSchedule("Case X", SCHEDULE_EDD, lmp, formSubmissionId);
        verify(scheduler).enrollIntoSchedule("Case X", SCHEDULE_LAB, lmp, formSubmissionId);
        verify(scheduler).enrollIntoSchedule("Case X", SCHEDULE_TT_1, lmp, formSubmissionId);
        verify(scheduler).enrollIntoSchedule("Case X", SCHEDULE_IFA_1, lmp, formSubmissionId);
        verify(scheduler).enrollIntoSchedule("Case X", SCHEDULE_HB_TEST_1, lmp, formSubmissionId);
        verify(scheduler).enrollIntoSchedule("Case X", SCHEDULE_DELIVERY_PLAN, lmp, formSubmissionId);
    }
}
